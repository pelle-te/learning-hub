...
- **오늘 학습** — *학습방법론 실행 레이어.* 블록 내부 4단계 흐름(개념 정찰→풀이→스케치→3문장 요약)·70% 룰. 블록마다 3문장 요약(3절)·CBMS 오답(6절)·보충 필요 백로그(5절)를 바로 기록.
- **통계 · 주간 리뷰** — 진행·예상 종료일·챕터 타임라인 + **인출 증거**(CBMS 추세·백지복습 완료율 — 투입 아닌 출력 지표), 주 1회 메타인지 점검(10절).
- **숙달도 지도 · 연동 현황** — 볼트/Anki 정본 인덱스와 지식엔진 산출(`_지식상태.json`)을 소비. 볼트 폴더 읽기는 프런트의 File System Access가 직접 한다(서버 없이도 동작).
