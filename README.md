...
- **⏰ 일과 & 가용시간** — 시작일/모듈길이/복습비중 설정, 수면·식사·취미·수업·공부 블록 → 요일별 공부시간·빈 시간.
- **📊 통계** — 과목별 진행·완료 챕터·예상 종료일, 주별 학습시간 막대, "무엇을 언제 배우는지" 챕터 타임라인.
- **📚 볼트 현황** — 전공 폴더 스캔(노트/검증/Anki 상태). 과목을 챕터까지 통째로 학습 항목에 추가.
- **🃏 Anki 현황** — _anki/*.txt 카드 집계 + AnkiConnect 실시간 due. '매일' 항목으로 추가.
