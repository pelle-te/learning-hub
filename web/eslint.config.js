...
export default tseslint.config(
    ...
    rules: {
      /* 인지복잡도 **래칫** — 임계는 현재 최댓값에 맞춰져 있고 **내려가기만 한다**.
         ...
         **`phone/ReviewView` 77→38** 을 더해 갚았다 — 실측 최댓값이 **66**(TodaySignature)이 됐고
         ...
         ⚠ (2026-08-20 정정: 그 귀속은 **더 이상 사실이 아니다** — 2회차가 `TodaySignature` 를
         갈라 705 로 내렸고 `max-lines` 지배자는 이제 `Settings.tsx`(713)다. 이 문단은 그때의
         기록으로 남긴다.)
      ...
      /* ⚠⚠ **62 → 25 로 내린다 — 단, 초과 18개는 기한부 예외로 명시한다**(2026-08-20 리뷰 M-14).
         ...
         ⚠ `max-lines` 는 **이때는 안 건드렸다** — 재측정하니 한도 727 vs 실측 720 이라 여유 1% 로
         이미 조여져 있었다(리뷰가 이 축을 함께 지적한 것은 과했고, 실측이 그 지적을 기각했다).
         ⚠⚠ **그리고 2회차가 그 조건을 바꿨다**(2026-08-20 · 아래 `max-lines` 블록 참조): 컴포넌트
         일곱을 갚으며 실측 최댓값이 720 → **713** 으로 내려가 여유가 1% → 2% 로 벌어졌고, 그래서
         **727 → 716 으로 함께 내렸다.** 위 문장을 "이 축은 영원히 안 건드린다"로 읽지 말 것 —
         그때의 *실측*에 대한 판정이었지 축에 대한 면제가 아니다. */
     ...
     여유 3줄은 의도다 — 인지복잡도 래칫(77·여유 0)과 같은 규율이고, 넘으면 쪼개라는 뜻이다. */
  {
    ...
    /* ⚠⚠ **727 → 716 으로 조인다(2026-08-20 · 인지복잡도 2회차의 부수 효과).**
       이 래칫의 계약은 바로 위 주석이 적은 대로 *"임계는 **현재 최댓값**이고 내려가기만 한다 ·
       여유 3줄은 의도"* 다. 그런데 같은 날 `TodaySignature` 를 갈면서 실측 최댓값이 **720 → 713**
       (`Settings.tsx`)으로 내려갔고, 한도를 그대로 두면 여유가 3 → **14** 로 벌어진다.
       즉 **내가 갚은 만큼 게이트가 헐거워지는** 형태이고, 이 저장소는 그것을 이미 두 번 이름
       붙였다(CLAUDE.md W4 _"큰 삭제 뒤 한도를 함께 내린다"_ · `bundle-budget.mjs` _"감소 방향에도
       똑같이 적용한다"_). 규율은 축마다 따로 배우는 것이 아니다.
       ⚠ 지배자가 `TodaySignature` 에서 **`Settings.tsx`(713)** 로 바뀌었다 — 다음에 여기를 더
       내리려면 그 파일을 봐야 한다(옛 주석의 귀속을 손으로 베끼면 또 틀린다 · F5 의 교훈). */
    rules: { 'max-lines': ['error', { max: 716, skipBlankLines: true, skipComments: true }] },
  },
