...
/* ── 탭의 역할(D-4) ─────────────────────────────────────────────────────
   ...
   · `view`        — **호스트 안의 이름 있는 뷰**(I048 · 2026-08-22). 나브·링·`g` 에는 안 나오고
                     **⌘K·딥링크·원장 집계**를 갖는다. `to` 가 그 뷰의 주소다.
                     ⚠⚠ **이 자리에 `retired`(은퇴)가 있었고, 그 어휘가 문제였다.** 「은퇴」는
                     *언젠가 판정한다*는 뜻이라 판정이 영원히 미뤄졌다 — `tabs.ts` 자신이 그
                     진단을 적어 두고도 어휘는 남겼다. `view` 는 유예가 아니라 **확정된 배치**다:
                     이 화면은 호스트 안에 산다. 자리를 옮기려면 `lens` 로 올리거나 지운다.
...
/**
 * 캘린더 **기간** — 주소가 소유한다(C029 · 2026-08-22).
 *
 * ⚠⚠ **`?view=` 가 아니다.** 이 저장소에서 `?view=` 는 «호스트 안의 다른 화면»이고 불변식 ⑱이
 * 그 값이 로스터에 있기를 요구한다. 일·주·월은 **같은 화면의 다른 기간**이라 로스터 행이 아니다 —
 * 셋을 등재하면 ⌘K 와 방문 집계가 화면 아닌 것으로 부풀고, 면제로 빼면 ⑱이 그만큼 헐거워진다.
 * ⚠ 어휘가 여기 사는 이유: 보내는 쪽(`features/today`)과 받는 쪽(`features/schedule`)이 **다른
 * feature** 라 서로를 import 할 수 없다(레이어 경계). 주소 어휘는 셸이 소유한다.
 */
export const SPAN_PARAM = 'span';
export const SPANS = ['day', 'week', 'month'] as const;
export type SchedSpan = (typeof SPANS)[number];
...
export interface TabMeta {
  ...
  segLabel?: string;
  /** `role:'view'` 전용 주소(Q-22 · I048). 호스트 안의 뷰는 `key` 가 경로가 아니다. */
  to?: string;
...
export const TABS: TabMeta[] = [
  ...
  /* ⚠⚠ **`goals`(내 길) 행이 여기 있었다 — 삭제됐다**(W4 · 발산 6회차 · 2026-08-07 · IA 판정표).
     W9 이 `/degree?view=path` 로 흡수하며 `role:'view'`(옛 `retired`) 를 붙였는데, 그 어휘의 뜻은 *"화면이
  ...
  /* ⚠⚠ **`journal`(학습 기록)이 여기 있었다 — 삭제됐다**(I048 · 2026-08-22 발상 축).
     N-12 가 `/day` 로 접었고 어휘(`retired`)가 그 사실을 담고 있었는데, 그 항목의 `to` 는
     **`/day` 자기 자신**이었다 — 즉 아래 `day` 탭과 **같은 화면을 가리키는 두 번째 이름**이다.
     흡수된 뷰(`?view=`)와 다르다: 저기는 이름이 없으면 그 화면을 못 부르지만, 여기는 이미
     레일에 자기 줄로 서 있는 화면의 별명일 뿐이다. 이름이 둘이면 다음 사람이 「둘은 다르다」로
     읽는다(U031 이 `TABS_EMPTY` 별칭에서 같은 판정을 받았다).
     복구: `git show <이 커밋의 부모>:web/src/shell/tabs.ts`. */
  ...
  /* ⚠⚠ **`graph`(학습 구조도)가 여기 있었다 — 삭제됐다**(I044 · 2026-08-22 발상 축).
     ...
     가득** 그린다 — 이 앱의 정의인 「한눈에」의 반례다. 선수 관계는 텍스트 한 줄이면 되고,
     목록(`/items`)이 같은 데이터를 더 조밀하게 말한다.
     복구: `git show <이 커밋의 부모>:web/src/features/graph/Graph.tsx` 외 5파일. */
  ...
  /* ⚠⚠ **W7(2026-08-02) — 은퇴를 시도했다가 되돌렸다. 불변식이 옳았다.**
     `role:'view'`(옛 `retired`) 로 바꿔 봤더니 불변식 ②가 즉시 잡았다: _"은퇴한 탭은 **착지 경로(to)** 를
  ...
  /* ⚠⚠⚠ **그러나 위 문단은 더 이상 현재가 아니다 — `guide` 는 되돌아왔다**(I025 · `2e28465` ·
     2026-08-22 09:51). 로스터 행이 `:385` 에 `role:'view'` 로 서 있다. 이유는 삭제가 틀렸다는
     것이 아니라 **행을 지우면 셋을 한꺼번에 잃기** 때문이다: 이름(`routeLabelOfLocation` 이
     호스트 이름을 읽는다) · ⌘K(팔레트가 로스터를 순회한다) · 관측(`visitKeyOfLocation` 이
     호스트로 접어 센다). 그래서 **지금의 선례는 「지운다」가 아니라 「`role:'view'` 로 내린다」**다.
     집행자는 **불변식 ⑱**(`?view=` 로 가는 화면은 전부 로스터에 있다)이고, 그것이 면제 문법을
     가지므로 — 다음 사람이 이 파일에서 정반대의 두 선례를 만나 **면제 사유를 적어 통과시키는
     쪽**으로 가지 않게 이 문단을 둔다(C049 · 2026-08-22). */
  ...
  /* ⚠⚠ **명사 주소 셋(`chapter`·`week`·`exam`)이 여기 있었다 — 삭제됐다**(I047 · 2026-08-22).

     N-12 가 셋을 만든 논거(*"링크로 못 가리키는 것은 ⌘K 로도 못 찾고 알림도 못 데려간다"*)는
     참이었지만, 만든 뒤 **아무도 그 명사를 가리키지 않았다**: 저장소 전수로 이 셋을 향해
     `navigate`·`<Link>` 하는 코드가 0이고, `route_visits` 에도 5키 전량 0행이다. 즉 주소는
     생겼는데 **그 주소를 쓰는 화면·알림·⌘K 항목이 하나도 안 생겼다** — 세 리다이렉트가 남은
     유일한 소비처였고, 그건 자기 자신이다.
     ⚠ `subject`·`mini` 는 남는다: 둘은 실제 호출부가 있다(⌘K·과목 카드·집중 세션).
     복구: `git show <이 커밋의 부모>:web/src/app/NounRoutes.tsx`. */
];
