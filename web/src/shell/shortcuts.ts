...
export const GLOBAL_SHORTCUTS: { keys: string; label: string }[] = [
  ...
  /* ⚠ 라벨을 짧게 유지한다 — 키 칩(`Ctrl + Shift + Space`)이 넓어서 긴 라벨이 **3줄로 감긴다**
     (실렌더로 확인 · `shortcuts-help-dark` 스냅샷이 이제 그걸 지킨다). 조건("등록 시")은 남긴다:
     이 항목은 유일하게 **등록이 실패할 수 있는** 단축키이고, 치트시트가 "된다"고 단정하면
     E16 이 없애려던 거짓말이 된다. 실패 사유는 연동 탭의 HOTKEY 채널이 말한다. */
  { keys: `${MOD_LABEL} + Shift + Space`, label: '앱 밖에서도 캡처 — 등록 시' },
];
