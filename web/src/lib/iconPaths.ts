...
export const ICON_PATHS = {
  ...
  // ⛔ `compass` 아이콘이 2026-08-29 에 빠졌다 — 유일 소비처였던 「내 길 지도」·안내 「목표·연관성」 절이 함께 은퇴했다(부모 목적 정정).
  /* ⚠ **`discovery`(발견 — 큰 별+작은 별)를 지웠다**(C059 · 2026-08-22). 마지막 소비처가
     `guide` 본문의 §③「수집·발견」이었는데 그 절이 부르던 탭 넷이 `TABS` 에 0건이라 절째로
     걷었다. 종전 이 자리 주석은 *"발견 **큐**는 `survey/` 로 갔지만 이 글리프는 `guide` 본문이
     계속 쓴다"* 였다 — 그 마지막 세입자까지 나간 것이다. 함께 있던 `radio`(진로 지도)도 같은
     경로로 지워졌고, 매번 `icons.test.ts` 의 '죽은 아이콘 0' 단언이 시끄럽게 잡았다. */
  ...
  // 탐색·검색 — 🔎 🔍 (🔗 은 위 link. `telescope` 는 탐구 수집 화면과 함께 P10 W4 에서 삭제)
  // 정보 — ℹ
