/* ============================================================
   ...
   **자기가 렌더되지 못하는 트리 안에** 있으므로 정의상 이 상황을 못 잡는다.
   ...
   종전에는 두 체인 모두 **종단 `.catch()` 가 없었다** — 즉 사용자에게 남는 복구 행동이 **0**
   이었다(관측과 전달이 짝이라는 `updater.rs` 의 명제가 여기서 반쪽이었다). 이 화면이 그 짝의
   전달 쪽이다.
   ⚠ **여기 있던 「텔레메트리가 `unhandledrejection` 으로 서버에 보냈다」는 이제 거짓이다**
   (C060 · 2026-08-22). `lib/telemetry.ts` 는 I052 가 걷었고(`7085303`) 저장소 전체에
   `unhandledrejection` 핸들러는 **0건**이다. 남겨 두면 «비동기 실패를 어딘가는 보고 있다»고
   읽히는데 실제로는 아무 층도 안 본다 — 그 구멍은 코드 축 1회차가 **R2** 로 따로 세웠고,
   집행자는 `C051`(타입 인지 비동기 린트)이다. 낡은 주석이 구멍을 가리는 것이 이 삭제의 이유다.
