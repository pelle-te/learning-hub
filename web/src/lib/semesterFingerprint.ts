 ...
 *
 * ⚠⚠ **여기 있던 것은 `hash32` 의 비트 사본이었다**(C046 · 2026-08-22 — 상수 `2166136261`·
 * `16777619` 가 `0x811c9dc5`·`0x01000193` 이다). 즉 SSOT 를 선언한 주석이 있는데 그 옆에
 * 같은 알고리즘이 손으로 한 벌 더 적혀 있었다. 대가는 다음 변경이 치른다: `utils.hash32` 는
 * *"노브만 바꾸면 다음 부팅에 전부 갱신된다"* 를 근거로 승수 교체를 **장려하는데**, 그때
 * 이 파일만 옛 알고리즘에 남아 **두 화면이 같은 id 에 대해 다른 이야기를 시작한다.**
 * ⚠ 출력은 **비트 동일**이다(20만 입력 대조 · 2026-08-22) — 스냅샷 재생성이 필요 없었다.
 */
function hash01(id: string): number {
  return (hash32(id) % 10000) / 10000;
}
