...
/* ── ⚠⚠ **세입자가 0이다**(P10 W4 · 2026-08-07) ────────────────────────────────
   ...
   ⚠⚠ **여기 있던 ✅「세입자가 돌아왔다 — `ics:feed`」문단을 지웠다**(C042 · 2026-08-22).
   그 세입자는 **I050 이 걷었다** — `lib/icsFeed.ts`(문단이 *"값의 형태를 소유한다"* 고 지목한
   파일)는 **존재하지 않고**, 살아 있는 `.ics` 피드는 라우트째 은퇴했다. 즉 위 두 문단이
   «비었다·그 대가는 이렇다» 라고 정확히 적어 둔 상태로 되돌아왔는데, 그 아래에서 ✅ 가
   **정반대**를 선언하고 있었다. 인수인계받는 사람은 살아 있는 D1 동기화 KV 가 있다고 믿고
   그 위에 기능을 얹는다 — 낡은 주석이 사실과 다르게 말하는 형태라 «없는 문서보다 나쁘다».
   ⚠ **배관은 존치한다** — 근거는 위 「왜 배관은 남기나」 문단 그대로이고 바뀐 것이 없다.
   ⚠ 실 DB `docs` 표에 행 3개가 남아 있는 것은 **데이터 축 소관**이다(`D023` 계열).
   다음 저작물 키가 생기면 여기 한 줄이면 된다 — 그때 ✅ 대신 «무엇이 그 값을 소유하는가»를
   같은 커밋에서 함께 적어라(이 문단이 지워진 이유가 그것이다). */
