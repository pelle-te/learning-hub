...
export async function enrollDevice(baseUrl: string, code: string, name: string): Promise<CloudConfig> {
  ...
  const res = await fetch(`${base}/api/enroll/claim`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ code, name }),
  });
  if (!res.ok) throw new Error(`등록 실패(${res.status}) — 코드가 틀렸거나 만료됐습니다.`);
  const j = (await res.json()) as { deviceId?: unknown; refreshToken?: unknown };
...
async function accessToken(cfg: CloudConfig, force = false): Promise<string> {
  ...
  const res = await fetch(`${cfg.baseUrl}/api/token`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ deviceId: cfg.deviceId, refreshToken: cfg.refreshToken }),
  });
  if (res.status === 401) {
    _access = null;
  ...
  const j = (await res.json()) as { accessToken?: unknown; expiresIn?: unknown };
...
async function authed(cfg: CloudConfig, path: string, init: RequestInit = {}): Promise<Response> {
  ...
  let res = await send(await accessToken(cfg));
  if (res.status === 401) {
    // 액세스 토큰 만료는 정상 경로다. 갱신해서 한 번 다시 친다.
    res = await send(await accessToken(cfg, true));
    if (res.status === 401) {
...
export async function pullChanges(cfg: CloudConfig, since: number, limit = 200): Promise<OutboxBatch> {
  ...
  const parsed = OutboxBatchSchema.safeParse(await res.json());
  if (!parsed.success) {
