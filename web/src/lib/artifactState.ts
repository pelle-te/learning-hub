...
/* ⛔⛔ **`?__state=` 개발용 상태 덮개를 여기 넣었다가 되돌렸다**(U077 · 2026-08-31 · **기각**).

   제안의 동기는 참이었다: 이 회차의 발견 다섯이 «아무도 그 화면을 못 열어서» 살아남았다.
   그리고 형태도 최소였다 — 판정 한 곳만 덮으면 화면·컴포넌트는 0줄 변경이다.

   ⛔ **그런데 불변식 ㉔(C058)이 정확히 그것을 금지한다**: _"`src/` 에 dev/prod 분기가 없다 —
   dev 에서만 도는 코드가 생기면 **그것을 보는 층이 하나도 없다**"_. 실제로 이 덮개를 심고 나서
   그 주석에 «트랙 A 는 프로덕션 빌드라 e2e 는 이 문을 못 쓴다» 고 적었는데, **그 문장 자체가
   ㉔ 이 말하는 결함의 정의**였다. 집행자가 곧바로 잡았다(`invariants.test.ts`).

   ⚠ 그래서 임계를 풀지 않고 **제안을 접었다.** 이건 심각도 없는 진보 항목이고, 검증망 쪽의
   필요는 `bootNoArtifacts`(U063)·`bootArtifactPhase` 가 **진짜 실패 경로로** 이미 덮는다 —
   남는 이득은 «사람이 눈으로 볼 때의 편의» 하나인데, 그것 때문에 못박힌 불변식을 무르지 않는다.
   ⭐ 다시 하려면 **모드를 안 보는 형태**여야 한다(그때 새 ID 를 받는다). */
