/* ============================================================
   atlasData.ts — 진로 지도의 **시드 데이터**(대분류 8 + 분야 25). 순수 상수만, 로직 없음.

   왜 분리했나: atlas.ts가 930줄이었는데 그 중 795줄이 이 데이터였다 — 파일 이름은 '로직 모듈'인데
   내용은 사실상 데이터 파일이라, 75줄짜리 실제 로직이 스크롤 저편에 묻혀 있었다.
   타입은 atlas.ts가 계속 소유하고 여기선 `import type`으로만 가져온다(런타임 순환 없음:
   런타임 의존은 atlas → atlasData 한 방향뿐).

   ── 데이터 성격 ──
   • FIELDS/CATEGORIES = 편집 가능한 '지식 골격'(시드·도메인 지식 기반 초안). 자동 수집(RSS) 전 손으로 채운다.
   • trends(동향) = 시드 예시. 후속에서 /api 자동 수집분이 필드별로 태깅되어 병합된다.
   • skills = '필요 역량'(진행도는 넣지 않는다 — 근거 없는 % 대신 필요 목록만).
============================================================ */

import type { AtlasCategory, AtlasField } from './atlas';
