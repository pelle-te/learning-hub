...
export default function SyllabusIntake() {
  ...
  const apply = (): void => {
    ...
    /* ⚠ **고른 눈금이 한 건도 안 들어갔으면 그 사실을 말한다.** 학기 개강일이 없으면
       `activeSemester` 가 null 이라 눈금이 갈 곳이 없는데, 종전엔 그냥 「눈금 0」이라 적혀
       사용자가 «내가 안 골랐나»로 읽었다(무음 실패의 전형). */
    ...
    <div className="ds-rule">
      <div className="mb-3 flex flex-wrap items-baseline gap-3">
        ...
        {sem ? (
          ...
          /* ⚠⚠ **종전 이 문구는 절반만 말했다**(2026-08-22 실측). 학기 날짜가 없으면 못 읽는 것이
             날짜만이 아니라 **학사 눈금이 들어갈 자리 자체가 없다** — `activeSemester` 가 `startDs`
             없는 학기를 안 고르므로(그 함수의 계약) 적용 단계에서 조용히 0건이 된다.
             실 데이터에서 학기 3개 중 **날짜가 있는 것이 0개**였고, 그 상태로 「눈금 2 반영됨」이
             아니라 「눈금 0 반영됨」이 뜬다 — 사용자는 무엇이 빠졌는지 알 방법이 없다.
             그래서 **무엇을 해야 하는지**를 말한다. */
          <Pill tiny tone="warn">
            학기 개강일이 없어요 — 연도 없는 날짜(3/2)도, 학사일정도 못 들어갑니다(졸업 계획 → 학기에서 개강일 입력)
          </Pill>
        )}
      </div>
