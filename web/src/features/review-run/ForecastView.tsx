...
export default function Forecast() {
  ...
  /* ⚠ **두 상태가 같은 이름을 갖는다**(U049 · 2026-08-31). 위 빈 상태는 `<section
     aria-label="복습 부하 예보">` 인데 이쪽은 라벨 없는 `<div>` 였다 — 그래서 «이 화면이 떴다»를
     묻는 검사가 한쪽 상태에서만 성립했고, 그 사각을 메우려고 앱 전역의 `h1`(워드마크)을
     `ready` 로 쓰던 것이 **다른 화면을 통과시키는** 기전이 됐다. 이름은 상태가 아니라 화면의
     것이므로 양쪽이 같이 진다(랜드마크가 하나 느는 것은 덤이다). */
  return (
    <section className={WRAP} aria-label="복습 부하 예보">
