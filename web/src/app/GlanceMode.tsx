...
export default function GlanceMode() {
  ...
  /* ⚠⚠ **`role="dialog" aria-modal` 을 선언해 놓고 그 계약을 하나도 안 지키고 있었다**
     ...
     ⚠ 두 훅은 이미 있었다(`useFocusTrap`·`useScrollLock`) — 이 화면만 그 목록 밖이었고,
     a11y 로스터(`_fixtures.ts`)에도 안 들어 있어 axe 가 볼 기회조차 없었다.
     ⚠⚠ **여기 「오버레이 6종이 쓴다」라고 적혀 있었다 — 뺐다**(V023 · 2026-08-22). 손으로 적은
     수이고 실제로 표류했다(실측 8 · 그중 하나가 H-11 이 고친 **이 화면 자신**이다). 이 저장소에서
     소비처 수는 늘어난 전력이 있고, 수를 고쳐 적으면 다음 회차가 같은 것을 또 발견한다 —
     세는 법은 `rg -l useFocusTrap src` 다(`렌즈-오버레이.md §1-A` 의 규율). */
