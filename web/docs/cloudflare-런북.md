...
### 7-5. 레이트 리밋 — **주 방어가 코드로 들어왔다** (2026-07-25 개정)

> ⚠ **이 절의 결론이 뒤집혔다.** 옛 제목은 _"주 방어는 코드가 아니라 대시보드다"_ 였고, 절차는 대시보드에서 WAF 규칙을 손으로 만드는 것이었다. 진단(아이솔레이트 로컬 카운터로는 부족하다)은 맞았지만 처방이 이 저장소의 원칙과 어긋났다 — 실제 방어가 **버전관리도 리뷰도 테스트도 안 되는 곳**에 살았다. `stylelint`·번들 예산·커버리지 임계를 들일 때 반복해 내린 결론(_"규약을 관습에 두면 흘러내린다"_)이 여기만 적용되지 않고 있었다.

**지금 방어는 두 겹이고 둘 다 코드다:**

| 층                    | 어디                                   | 무엇을 막나                            | 왜 남겨 두나                                                |
| --------------------- | -------------------------------------- | -------------------------------------- | ----------------------------------------------------------- |
| ① in-memory 카운터    | `server/src/index.ts` `rateLimited()`  | 같은 아이솔레이트가 보는 명백한 폭주   | 바인딩이 없는 환경(로컬 `wrangler dev`·테스트)에서도 막는다 |
| ② `ratelimits` 바인딩 | `server/wrangler.jsonc` `AUTH_LIMITER` | **아이솔레이트 경계를 넘는 분산 요청** | 이게 주 방어. 커밋에 남고 PR 에서 보이고 롤백된다           |

- 설정값: **20 requests / 60s / (IP + 경로)**. 옛 in-memory 값을 그대로 승계했다(의미 보존).
  - ⚠ `simple.period` 는 스키마상 **10 또는 60 만** 허용된다(`node_modules/wrangler/config-schema.json` 실측).
- 적용 대상: `/api/enroll/*` · `/api/token` · `/api/log` — **무인증 라우트만**.
- ⚠ `/api/sync/*` 에는 **걸지 말 것.** 이미 토큰이 필요하고, 여기 걸면 첫 전량 동기화(여러 배치로 나뉜다)가 자기 규칙에 막힌다.
- ⚠ 바인딩이 없거나 `limit()` 이 실패하면 **통과시킨다(fail-open)**. 레이트 리미터 장애가 곧 서비스 장애가 되는 것은 이 앱의 위험 프로파일에 안 맞는다 — 여기서 지키는 것은 남용이지 기밀이 아니다(인증은 별개 층이고 그쪽은 fail-closed 다).

**검증**: `test/roundtrip.test.ts` 의 `무인증 라우트 레이트 리밋` 이 실 workerd 에서 25연타를 쳐 20통과/5차단과 **IP 별 버킷 격리**를 단언한다. ⚠ 종전에는 이 동작에 테스트가 **하나도 없었다**.

**대시보드 WAF 규칙은 이제 선택이다.** 바인딩이 같은 일을 하므로 중복이다. 계정 단위로 더 위쪽(워커 실행 전)에서 끊고 싶을 때만 추가하고, 그때도 **이 문서가 아니라 규칙 자체가 SSOT 가 되지 않게** 여기에 값을 적어 둘 것.
...
| **P1-7** 요청 크기 상한·RL       | **§6-2**(배치 상한) · §7-5 · §8-2 | ✅ 1MB 본문 상한(2겹) + 무인증 라우트 RL. **주 방어 = `ratelimits` 바인딩**(§7-5) |
