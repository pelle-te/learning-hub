...
### 5-1b. ⚠⚠ **유출됐을 때** — 무엇을, 어떤 **순서**로 (D015 · 2026-08-21 데이터 축)

이 문서는 시크릿을 **설정하는 법**만 적고 «샜을 때 어떻게 되돌리나»는 적지 않았다. 5종 중 그
절차가 있는 것은 **업데이터 개인키 1종뿐**이었다(`릴리스.md`). 급할 때 필요한 것은 셋이다 —
**무엇을 먼저 하나 · 그동안 무엇이 죽나 · 순서를 틀리면 왜 소용없나.**

| 유출된 것                                      | 즉시                                                                   | 그다음                                          | 잃는 것 / 함정                                                                                                                                                                                               |
| ---------------------------------------------- | ---------------------------------------------------------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **`HUB_SIGNING_KEY`**                          | `wrangler secret put HUB_SIGNING_KEY`(새 난수)                         | 없음 — 기기가 다음 요청에서 알아서 재발급받는다 | ⚠ **거의 공짜다.** 죽는 것은 발급된 ≤15분짜리 액세스 토큰뿐이고, 리프레시 토큰은 D1 의 **해시**라 이 키와 무관하다. 즉 사용자는 재로그인도 재등록도 안 한다 — **망설일 이유가 없다는 것이 이 줄의 요점이다** |
| **`HUB_ADMIN_KEY`**                            | `wrangler secret put HUB_ADMIN_KEY`(새 난수)                           | `DELETE FROM enroll_codes` (미사용 코드 무효화) | ⚠ **순서가 계약이다.** 키를 먼저 바꾸지 않고 코드만 지우면 공격자가 **곧바로 다시 발급**한다. 잃는 것: 발급해 두고 안 쓴 등록 코드                                                                           |
| **기기 리프레시 토큰**(한 대)                  | 앱 › 설정 › 클라우드에서 그 기기 **폐기** (`POST /api/devices/revoke`) | 그 기기에서 등록 코드로 **재등록**              | 폐기는 액세스 토큰까지 즉시 막는다(`requireDevice` 가 매 요청 확인한다 — §5-2 ④). 잃는 것: 그 기기의 미전송 아웃박스는 **로컬에 남고** 재등록 후 올라간다                                                    |
| **Cloudflare API 토큰**(`wrangler login` 자격) | dash.cloudflare.com › My Profile › API Tokens 에서 **Revoke**          | `wrangler login` 재인증                         | ⚠ 이건 **계정 전체**다 — 배포·D1·시크릿을 전부 쥔다. 폐기 후 `wrangler secret list` 로 남의 시크릿이 바뀌지 않았는지 확인할 것                                                                               |
| **업데이터 개인키**                            | —                                                                      | **`web/docs/릴리스.md` 가 SSOT**                | ⚠ **재생성 불가.** 유출 = 업데이트 사칭, 분실 = 영구 배포 불가. 여기 사본을 두지 않는다                                                                                                                      |

> ⚠ **§5-2 의 발급 curl 은 `HUB_ADMIN_KEY` 를 명령행에 그대로 친다** — 셸 히스토리·화면 공유·
> 스크린샷에 남는 형태다. 남기고 싶지 않으면 환경변수로 우회한다:
>
> ```sh
> read -rs HUB_ADMIN_KEY          # 화면에 안 찍힌다
> curl -s -X POST https://<워커>.workers.dev/api/enroll/new >   -H "Authorization: Bearer $HUB_ADMIN_KEY"
> ```
...
**검증**: `server/test/roundtrip.test.ts` 의 `무인증 라우트 레이트 리밋` 이 실 workerd 에서 25연타를 쳐 20통과/5차단과 **IP 별 버킷 격리**를 단언한다. ⚠ 종전에는 이 동작에 테스트가 **하나도 없었다**.
