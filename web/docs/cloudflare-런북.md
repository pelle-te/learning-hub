...
> **호스트 변경**(사용자 결정 2026-07-19): **Oracle Cloud VM → Cloudflare Workers + D1.** `oracle-런북.md` 는 **이력으로 남기고 실행하지 않는다.**
...
`oracle-런북.md` §9-2 가 이렇게 적었다: _"Workers+D1 을 택했다면 D1 스키마·바인딩·Workers 런타임 가정이 전부 이사 비용이었다."_
...
4. 새 호스트: ⓐ 다른 서버리스(Deno Deploy 등 — Hono 라 라우팅 재사용) ⓑ 유료 VPS(그때 `oracle-런북.md` §4~§7 이 **다시 유효해진다** — 그래서 그 문서를 지우지 않는다)
