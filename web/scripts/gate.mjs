#!/usr/bin/env node
/* ============================================================
   gate.mjs — web 품질 게이트 러너. verify(+e2e)를 돌려 압축 요약만 출력.
   서브에이전트/슬래시 명령이 전체 로그 대신 짧은 통과/실패 보고를 받게 한다.
   사용: cd web && node scripts/gate.mjs [quick]   (quick=verify만, e2e 생략)
...
if (!quick) steps.push(['e2e', ['run', 'e2e']]);
...
if (quick) console.log('(quick 모드 — e2e 생략)');
