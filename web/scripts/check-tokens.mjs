...
console.log(
  `✓ CSS 변수 참조 ${참조.size}종 전부 정의됨(선언 ${선언.size}종) · tokens.css 미사용 0(원장 ${미사용_원장.length}건) · @theme 고아 0(원장 ${브리지_원장.length}건) · ds-* 고아 0(원장 ${ds_원장.length}건) · 미선언 사다리 칸 0 · 반픽셀 ${반픽셀.length}/${반픽셀_래칫} · *.module.css 0개 · 카드 표면 ${카드표면.length}/${카드래칫}(원장 ${카드원장.size}파일) · TS 생 hex 0(원장 ${hex결과.원장}파일).`,
);
