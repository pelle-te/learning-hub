...
export default defineConfig({
  ...
  /* ⚠⚠ **워커를 1로 못박는다**(I049 실행 중 실측 · 2026-08-22).

     이 저장소는 베이스라인을 **`--workers=1` 로 생성**한다고 `package.json` 이 못박아 두고
     (`e2e:update`), 검사는 `fullyParallel` + 워커 무제한으로 돌렸다 — **만드는 조건과 재는
     조건이 달랐다.** 그 어긋남은 평소엔 `retries: 1` 이 흡수하지만 어긋남 자체는 남아 있고,
     이번에 레일 로스터가 한 줄 줄어들자(I048) 표면화됐다: `settings`·`integrations` 넉 장이
     **병렬에서만** 레일 글리프 래스터화가 갈렸고 단일 워커에서는 209장 전부 통과했다.
     계측으로 확인한 것 — 폰트는 두 조건에서 동일했고(`fonts.check` 참 · 항목 높이 38px 동일)
     갈린 것은 레일 nav 의 `scrollTop`(40 ↔ 43) 같은 서브픽셀 상태였다.

     ⚠ 대가는 벽시계다(약 50초 → 3.6분). 그 값을 치르는 이유: 임계를 늘리는 대안은 **드리프트를
     삼키는 쪽**이고(§15-7 이 몇 달을 잃은 그 형태), 재시도로 덮는 대안은 어긋남을 남긴 채
     «가끔 빨간불»을 상시화한다. 모션 설정이 같은 이유로 이미 `workers: 1` 이다.
     ⚠ 되돌릴 조건: 캡처가 서브픽셀 스크롤 상태에 의존하지 않게 되는 것(그때는 이 줄을 지우고
     병렬로 되돌린 뒤 넉 장이 통과하는지 **직접 확인**할 것 — 추측으로 되돌리지 마라). */
  workers: 1,
  fullyParallel: true,
