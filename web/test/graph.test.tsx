...
/* Graph(지식맵) 렌더 회귀 — 캔버스 시뮬레이션은 2D 컨텍스트 스텁 + reduced-motion(동기 레이아웃)
...
test('항목이 있으면 canvas(role=img)와 스크린리더 대체 목록을 그린다', () => {
  ...
  const canvas = screen.getByRole('img', { name: /지식맵/ });
...
test('항목이 없으면 1급 빈 상태(EmptyState)를 보여준다', () => {
  ...
  renderGraph();
  expect(screen.getByText(/아직 지식맵이 비어 있어요/)).toBeInTheDocument();
