...
/* ⚠⚠ **체크섬 핀** — 이 파일에서 가장 중요한 검사다.
   ...
   아래 값은 **실제 DB 에 저장된 체크섬**이다(2026-07-19 대조 확인).

   ⚠ **그 "대조 확인"은 손으로 한 번 옮겨 적은 것이었다 — 지금은 매 게이트가 실물과 대조한다**
   (2026-08-01). `src-tauri/src/db.rs` 의 `실_db_스모크_이_빌드가_사용자의_db_를_그대로_연다` 가
   사용자의 실 DB `_sqlx_migrations.checksum` 을 **읽기 전용**으로 읽어 번들 SQL 과 바이트 대조한다.
   즉 이 표가 지키는 명제는 "핀과 SQL 이 같다"이고, 저기가 지키는 명제는 **"이 빌드가 사용자의
   DB 를 실제로 연다"** 다. 둘 다 필요하다 — 여기는 실 DB 가 없는 기계(CI)에서도 돌고,
   저기는 표 자체가 표류하는 것을 잡는다.
...
const PINNED_CHECKSUMS: Record<number, string> = {
  ...
  // C-3(2026-07-30 `/감사 근본`) — summaries 키를 (sid,ord)→(sid,id). 기본키 변경이라 표를
  // 다시 만드는 유일한 마이그레이션이고, 그래서 텍스트 불변이 특히 중요하다(재실행 불가).
  9: '3804e06b0fba5eaecb55dc70',
  // W2(발산 6회차) — 홉 원장. 007·008 과 같은 부류(동기화 대상 아님)라 되돌릴 일이 없지만,
  // 핀은 "되돌릴 일이 있나"와 무관하다: 실 DB 에 적용되는 시점은 배포가 아니라 **앱을 한 번
  // 띄우는 순간**이고, 그래서 면제 구간이 실질적으로 존재하지 않는다(아래 케이스의 근거).
  10: '52a461fa25f0ee7ce36bf202',
  // W3(발산 6회차) — 유휴 원장. 010 과 같은 자리에서 같은 이유로 핀한다(면제 구간이 곧 구멍).
  11: '77133a84549f1e3ba6976be1',
};
