...
const load = vi.fn(async () => ({
  execute: async (sql: string, args: unknown[] = []) => {
    ...
    if (m) {
      ...
      /* ⚠ **다중 VALUES 를 이해해야 한다**(H26 · 2026-07-30). `coalesceStmts` 가 같은 SQL 의
         연속 INSERT 를 `VALUES (…),(…),(…)` 한 문장으로 접으면서, 첫 행만 읽던 이 더블이
         "쓰기는 됐는데 되읽기가 비어 있다"로 잡혔다 — 프로덕션이 아니라 **더블이 좁았던 것**이다.
         열 수로 인자를 잘라 각 묶음을 한 행으로 쓴다(진짜 SQLite 와 같은 의미). */
      for (let off = 0; off < args.length; off += cols.length) {
        const chunk = args.slice(off, off + cols.length);
        const row: Record<string, unknown> = {};
        cols.forEach((c, i) => (row[c] = chunk[i]));
        store.set(`${table}\x00${String(chunk[0])}\x00${cols.length > 2 ? String(chunk[1]) : ''}`, {
          ...row,
          __t: table,
        });
      }
