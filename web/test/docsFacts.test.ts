...
/* ⚠⚠ **접두 집합은 「소스를 담은 최상위 디렉터리 전량」이다** — 빠지면 그 구역이 통째로 침묵한다.
   `C075`(2026-09-02)가 그 형태였다: `.claude/`(훅·명령·에이전트 문서)가 여기 없어서, 그 구역을
   가리키는 경로가 사라져도 검사기가 조용했다. 되심기로 실증했다(`.claude/hooks/절대없는파일.sh`
   를 심었더니 **6 통과**). 새 최상위 디렉터리를 만들면 여기 한 줄을 함께 넣어라 — 세는 법은
   `ls -a` 로 루트의 디렉터리를 훑는 것이고, 지금 그 전량이 여기 있다.
   ⛔ **루트 「파일」(`package.json`·`CLAUDE.md`…)은 일부러 밖이다** — 접두가 없어 판정이 안 선다.
   실측(C075 회차): 백틱 안의 슬래시 없는 이름 **355 종 중 348 이 루트 파일이 아니었다**
   (`main.tsx` 같은 basename · `Math.imul`·`item.id` 같은 속성 참조 · `.card`·`1.5` 같은 클래스·수).
   즉 이 축을 열면 그날로 위반 348 이라 `.skip` 이 붙는다 — 머리주석의 «상대 경로는 판정이 안
   선다»가 루트 파일에도 그대로 적용된다. 루트 파일을 검사받게 하려면 문서가 **경로로** 적어야
   한다(예: `web/package.json`). */
const 루트접두 = ['web/', 'src-tauri/', 'server/', '.github/', '.husky/', '.claude/'];
