...
/* ============================================================
   recordEditor.test.tsx — useRecordEditor SSOT(인라인 편집 + 삭제-되돌리기).
   Journal 3카드가 공유하는 편집/삭제 기계 — 검증 abort·저장·삭제 후 위치(idx) 복원을 겨눈다
...
function opts(list: Row[]) {
  return {
    ...
    restore: (st: AppState, rec: Row, idx: number) => {
      (st as unknown as { rows: Row[] }).rows.splice(idx, 0, rec);
    },
    deleteLabel: '삭제됨',
    savedToast: '수정됨',
  };
...
describe('useRecordEditor', () => {
  it('startEdit이 editId·draft를 세팅, cancel이 초기화', () => {
    const { result } = renderHook(() => useRecordEditor(opts(getRows())));
  ...
  it('del이 제거하고, 되돌리기는 원래 위치(idx)로 복원', () => {
    let undo: () => void = () => {};
    vi.spyOn(toast, 'toastUndo').mockImplementation((_l, fn) => {
      undo = fn;
    });
    const { result } = renderHook(() => useRecordEditor(opts(getRows())));
    ...
    act(() => undo());
    expect(getRows().map((r) => r.id)).toEqual(['a', 'b', 'c']); // 끝이 아니라 제자리
  });
});
