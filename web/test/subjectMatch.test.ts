...
import { matchSubjectIndex, normSubject, subjectJoin } from '@/lib/subjectMatch';
...
describe('subjectJoin — 계측', () => {
  it('붙은 수와 안 붙은 이름을 함께 준다', () => {
    ...
    expect(r).toEqual({ total: 3, matched: 2, unmatched: ['통계'] });
  });

  it('후보가 0이면 전부 미연결 — "표기 불일치"가 아니라 "붙일 상대가 없다"', () => {
    const r = subjectJoin(['물리', '수학'], []);
    expect(r).toEqual({ total: 2, matched: 0, unmatched: ['물리', '수학'] });
  });
