...
const 알려진위반: Record<string, { 사유: string; 재검토: string }> = {
  '대비-light :: color-contrast': {
    사유:
      ...
      'accent 를 **lime** 으로 두므로 라이트 사용자 전원이 이 색을 본다. 수치는 `#4d7c0f` on `#ecf1e9`. ' +
      '⚠ 종전 사유는 여기에 "주석이 기본 violet 이라 적혀 코드와 어긋난다"를 함께 들고 있었다 — ' +
      '그 드리프트는 2026-07-26 에 해소됐고(주석이 lime 을 정확히 적는다), **사문화된 사유가 원장에 ' +
      '남아 있던 것**이다. 게이트는 만료일에만 반응하므로 이 종류는 감사만 본다(2026-07-30). ' +
