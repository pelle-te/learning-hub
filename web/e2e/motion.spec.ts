...
/* ============================================================
   ...
   **스냅샷 117장이 전부 통과한다** — 즉 모션 층은 시각 게이트가 원리적으로 못 보는 자리다.
   ...
   ## ⚠⚠ **이 층이 불변식 ⑥과 겹치는가 — 실측으로 답했다**(V030 · 2026-08-22)

   질문은 *"정적 집행자(불변식 ⑥)가 이미 막는 것을 스냅샷 4장이 겹쳐 막는가"* 였다. 반증 둘:

   | 바꾼 것 | 불변식 82종 | 모션 스냅샷 |
   | --- | --- | --- |
   | `--dur-slow` 340→204ms(길이 **값**) | **잡는다** | **잡는다**(enter·commit) |
   | `--pulse-r` 10→6px(키프레임 **기하**) | **전량 통과** | **잡는다**(맥동 클립) |

   → **겹치지 않는다.** 두 층이 재는 것이 다르다:
   · 불변식 ⑥은 **문법**을 막는다(어휘 밖 이름 · 시간 리터럴 · `duration-*` 브리지 누락).
     길이 값을 잡은 것은 «막아서»가 아니라 `--dur-slow` 가 `lib/motion.ts` 의 `COMMIT_MS` 와
     **짝이라는 별도 계약** 때문이다 — 그 짝이 없는 토큰이면 정적 층은 아무 말도 안 한다.
   · 스냅샷은 **값과 모양**을 막는다. 기하 변경은 문법을 하나도 안 어기므로 정적으로는
     원리적으로 안 보인다(위 표 둘째 줄이 그 증거다).

   ⚠ 그러니 «정적 검사가 있으니 스냅샷 4장을 줄이자»는 제안이 다시 오면 **위 표를 먼저 재라.**
   줄이는 순간 잃는 것은 «중복»이 아니라 **기하 축 전부**다.
