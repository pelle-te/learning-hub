...
⚠⚠ **이 가드는 이제 무장돼 있다**(2026-08-06 · 2026-08-01 감사가 처방한 주석 정정). 심을 당시엔
_"지금은 안전하다(릴리스 0건 = 다운그레이드할 대상이 없다) · 첫 업데이트를 내는 순간 무장된다"_
였는데, 그 뒤 **0.2.0·0.3.0·0.4.0 이 실제로 출하됐다** — 즉 문장이 서술한 조건은 벌써 지나갔고,
그 문장을 믿고 읽는 사람은 이 코드가 아직 가정적인 방어라고 오해한다. 지금 v0.3.0 exe 로
v0.4.0 이 만진 DB 를 열면 이 경로가 **실제로** 탄다. 처방은 조용한 폴백이 아니라 **명시적
화면**이고, 그 판정에 필요한 값은 "DB 에 적용된 최대 버전"과 "이 빌드가 아는 최대 버전"
둘뿐이다.
⚠ 여기 버전 번호를 손으로 더 적지 말 것 — 그 목록이 곧 다음 드리프트다(이 주석 자신이 그
사례였다). "출하됐다"는 사실만 남기고 개수는 `web/public/updates/latest.json` 이 정본이다.
