...
/// 로그 파일이 사는 폴더. override 가 있으면 **거기로 간다**.
///
/// ⚠⚠ **이 함수가 없던 동안 로그만 SD-6 밖이었다**(O007 · 2026-08-22 운영 축). 이 파일 머리말이
/// *"하네스가 자기 폴더를 지정하면 앱의 **모든** 사용자 데이터가 거기로 간다"* 고 선언하는데,
/// `tauri-plugin-log` 의 `TargetKind::LogDir` 은 `app_log_dir()`(= 식별자 기준)을 쓰므로
/// **override 를 안 탔다.** 그 자체로 사고는 아니었다 — 로그는 데이터가 아니니까. 그런데 O007 이
/// 프런트 `console.error` 를 그 파일로 잇자 **트랙 B 가 그것을 검사하려면 사용자의 실제 로그에
/// 써야 하는** 상태가 됐고, 그건 이 파일이 존재하는 이유(«검증이 검증 대상을 만지지 않는다»)와
/// 정면으로 어긋난다.
///
/// ⚠ `None` 을 돌려주면 호출부가 **플러그인 기본값을 그대로 쓴다** — 배포본의 경로는 한 글자도
/// 안 바뀐다(머리말 규율 1: override 는 순수 opt-in).
