...
## 래칫 감사 — **여유가 실측보다 벌어진 셋**(2026-08-20 · 3회차)

원장이 0이 된 뒤 남은 것을 찾다가, 이 세션이 반복해서 고친 실패 형태가 **게이트 자신에게** 남아
있는 것을 발견했다. 이 저장소는 그 형태에 이미 두 번 이름을 붙여 뒀다:

> *"큰 삭제 뒤 한도를 함께 내린다. 여유 33%면 게이트가 아무것도 안 지킨다"*(CLAUDE.md W4)
> *"한도를 올릴 때만 근거를 요구하고 내릴 때는 안 내리면, 큰 삭제 한 번마다 게이트가 조용히
> 헐거워진다"*(`bundle-budget.mjs`)

| 래칫 | 실측 | 한도(전) | 여유 | 한도(후) | 왜 벌어졌나 |
|---|---|---|---|---|---|
| `max-lines` | **713**(`Settings.tsx`) | 727 | 2% | **716** | **이번 회차 내 리팩터**가 `TodaySignature` 를 720→705 로 줄였다 |
| 카드 표면 총량 | **25** | 38 | **52%** | **25** | P10 W4 가 화면 다섯을 지웠는데 이 수를 안 내렸다 |
| `phone.html` css 예산 | **10.2** | 15 | **32%** | **12** | 같은 W4 커밋이 **js 만 내리고 css 를 빼먹었다** |

⚠⚠ **셋 중 둘은 같은 커밋(P10 W4)의 누락이고, 그 커밋 자신이 그 이유를 적어 뒀다.** `js` 한도를
내리며 _"한도를 그대로 두면 33% 여유가 되어 게이트가 아무것도 안 지킨다"_ 라 쓴 바로 그 판정이
같은 파일의 `css` 줄과 옆 파일의 카드 래칫까지 오지 않았다. **판정은 축마다 따로 배우는 것이
아닌데, 손으로 적용하면 축마다 따로 적용된다** — 이 저장소가 손베낌에서 반복해 물린 형태의
세 번째 판이다.

⚠ 셋째(`max-lines`)는 성질이 다르다: **내가 갚은 만큼 게이트가 헐거워진 것**이다. 부채를 갚으면
래칫을 함께 조여야 순이익이 된다 — 안 그러면 다음 사람에게 14줄의 무신호 성장분을 선물하고 끝난다.

⚠ **안 건드린 것도 적는다**: 데스크톱 css(31.6/37 · 14.6%) · 축 ③ 전체 총량 넷은 전부 정책
(~15%) 안이거나 그 언저리(16~22% · 절대차 1KB 미만)라 그대로 뒀다. 여기까지 조이면 무관한
변경에 빨간불이 뜨는 flaky 게이트가 되고, 그건 `bundle-budget.mjs` 가 명시적으로 피하려는 것이다.

⚠ 조인 둘은 **심은 위반으로 검출을 확인했다**: `Settings.tsx` 에 5줄을 더하면 718 > 716 으로
실패하고, 원장 파일에 카드 표면 하나를 더하면 26 > 25 로 실패한다(둘 다 제거 후 통과).

### ⚠ 그리고 `eslint.config.js` 가 자기모순이 됐다 — 함께 고쳤다

그 파일의 인지복잡도 절이 *"`max-lines` 는 **안 건드린다** — 여유 1% 로 이미 조여져 있다"* 라
적고 있었는데, 같은 파일 아래에서 그 한도를 내렸다. 이 저장소가 `features.css`↔`motion.css` 에서
물린 것과 같은 형태(**한 파일 안에서 서로 다른 말**)라, 그 문장을 *"그때의 실측에 대한 판정이었지
축에 대한 면제가 아니다"* 로 정정하고, 낡은 귀속(`max-lines` 지배자 = `TodaySignature`)도
`Settings.tsx` 로 바로잡았다.

## 남은 것

없음. 원 46건 + 정독 2건 + 기한부 원장 2건 + 래칫 셋, 전부 닫혔다. **열린 원장이 0 이고,
래칫 넷(인지복잡도·`max-lines`·카드 표면·번들 예산)이 전부 실측에 붙어 있다.**

⚠ **코드 밖에 남은 것 하나**: 이 회차의 커밋 셋이 아직 **푸시되지 않았다**. 이 저장소는 그
형태에 이미 물린 적이 있다 — *"코드는 건강한데 22 커밋 미푸시라 지난 4일이 사용자에게 도달하지
않았다"*(2026-08-06). 푸시는 바깥으로 나가는 행동이라 **사용자 승인 없이 하지 않는다.**
