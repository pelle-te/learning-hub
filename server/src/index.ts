...
/* ⚠⚠ **루트(`/`)는 폰으로 보낸다**(C047 · 2026-08-22 코드 축 1회차).
   ...
   종전엔 SPA 폴백이 루트에 **데스크톱 엔트리**(`index.html`)를 줬다. 그 엔트리는
   `enableBrowserDb()` 를 부르지 않으므로 브라우저에서는 `isSqlitePrimary()` 가 거짓이고,
   저장이 **localStorage 백엔드**로 흐른다. 아웃박스는 SQLite 만 훑는다 → 거기서 한 편집은
   **영원히 동기화되지 않는다.** 그리고 앱은 완전히 정상으로 보인다(배너도 경고도 없다).

   도달 경로가 실재한다: 북마크 · 주소 자동완성 · 공유 링크. 즉 «폰 웹앱을 쓰라»는 계약에
   **문이 하나 더** 열려 있었다.

   ⚠ 이 저장소의 브라우저 localStorage 폴백 **자체는 의도된 것**이다 — dev 루프와 트랙 A 전량이
   그 위에서 돈다. 지적은 «프로덕션 오리진에 그 문이 열려 있고 아무도 안 말한다» 였다.
   그래서 없애는 것이 아니라 **문을 옮긴다**: 데스크톱 SPA 는 명시 경로(`/today` 등)로만 열린다.
   ⚠ `302`(임시)를 쓴다 — `301` 은 브라우저가 영구 캐시해 되돌리기가 사용자 손을 탄다.
   ⚠ 짝: `wrangler.jsonc` 의 `run_worker_first` 에 `"/"` 가 있어야 이 라우트에 닿는다.
   ⚠⚠ 이 계약을 잠그던 테스트가 **정반대를 단언하고 있었다**(`test/assets.test.ts` 의
   *"`/` 가 index.html 을 준다"*) — 옛 계약을 테스트가 지키고 있었다는 것이 이 건의 절반이다. */
