...
- 슬래시 명령 `/게이트`가 verify+audit+build+budget+**server verify**+e2e(+cargo 있으면 tauri:fmt·tauri:clippy·tauri:check·cargo test·tauri:build·e2e:shell)를 돌려 압축 리포트만 반환한다(quick=verify만). **정본은 `web/scripts/gate.mjs` 머리주석**(이 줄은 손베낌이라 표류한다 — 실제로 두 번 표류했다).
...
- `web/docs/방향.md` — **"왜 만드나"의 SSOT** · `/개선 발산`의 1차 앵커(2026-07-31 신설). 제품 목적
