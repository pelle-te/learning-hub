...
> **폴더 기준점** — git 루트는 `작업 폴더/`(형제: `전공/`·`시스템/`·`anki/`·`참고자료/`·**`러닝허브/`**). 이 문서·경로는 **`러닝허브/` 기준**이다. 웹앱 소스는 전부 `web/`.
> **이 문서는 웹앱(러닝허브) 전용**이다. 볼트 노트 파이프라인 규약은 `../전공/CLAUDE.md`가 단일 진리 — 혼동 금지.

러닝 허브는 **볼트(전공/)·Anki·일과 데이터를 한눈에 보는 로컬 학습 대시보드**다. 두 프로세스로 동작한다:
