...
>
> ⚠ **새 수집·교양 기능을 hub 에 만들지 말 것.** 이 앱은 그 이유로 다섯 번 강등을 반복했다(H24·E17-IA·N-6·P-4·W9 — 전부 _"학습 상태 소비 0"_ 이 근거였다). 그 반복이 IA 문제가 아니라 **거처 문제**였다는 것이 P10 의 진단이다.
> ⚠⚠ **강등과 제거를 혼동하지 말 것**(W4 가 남긴 어휘 구분). 이 저장소엔 _"지우지 않는다 — 도달성만 회수한다"_ 는 규율이 있고(`markets` 강등 · `graph` 은퇴) 그건 **같은 앱 안에서 자리를 옮길 때** 옳다(⌘K·딥링크가 여전히 닿아야 하므로 코드가 살아야 한다). W4 는 **다른 필러로의 이사**라 남기면 두 벌이 되고, 그건 워크스페이스 제1원칙(사실마다 집 하나) 위반이다 — **도달성 손실이 의도**다. 되살리려면 `git show 1c21ad5:<경로>`.
...
npm run e2e:a11y # a11y — axe-core 로 렌더된 DOM 을 검사(`serious`+`critical` 만 실패).
                 ...
                 #   ⚠ **범위가 두 번 넓어졌다**(H5·H6 · 2026-07-30 → ux 축 · 2026-08-21).
                 #   종전엔 구조 검사만 전 화면이고 **라이트 대비는 `/today` 한 화면**뿐이었으며,
                 #   폰·오버레이·`/mini` 는 **한 번도 검사된 적이 없었다**(*경로로 도달하는* 화면만 봤다).
                 #   ⚠⚠ **여기 적혀 있던 「26 → 64」를 지웠다**(V028 · 2026-08-21): 네 줄 아래가
                 #   스스로 «화면 수를 여기 적지 않는다»고 말하는데 같은 문단이 수를 적고 있었고,
                 #   그 수는 실제로 낡아 있었다(실측 94 · 그마저 로스터가 자라면 또 바뀐다).
                 #   지금은 ① 구조(다크 전 화면) ② 라이트 대비(같은 목록) ③ 오버레이 ④ `/mini`
...
npm run budget   # 번들 예산 — 여러 축. ⚠⚠ **개수도 목록도 여기 적지 마라**(2026-08-28 정정):
                 #   이 자리에 「4축」이라 적혀 있던 동안 실제로는 **여섯**이 됐다(O019 가 저장소
                 #   이력 축을, P030 이 정적 자산 축을 붙였다). 정본은 `web/scripts/bundle-budget.mjs`
                 #   의 구획 주석이고, 세고 싶으면 그 출력의 `---` 줄을 세라.
                 # ⭐ 그 축들이 재는 **단위가 서로 다르다**는 것만 여기 남긴다 — gz(전송) ·
                 #   raw(파싱·실행) · 전송 바이트(정적 자산) · **개수** · MiB(저장소 이력).
                 #   「예산 녹색」을 근거로 쓰기 전에 **어느 단위로 녹색인지** 봐라(성능 축 R1).
                 # ⚠ `npm run budget:repo` 는 **저장소 이력 축만** 돈다(dist 불요) — CI 의
                 #   `freshness` 잡 자리다. 그 축은 `fetch-depth: 0` 위에서만 참을 말한다(P032).
...
- ⚠ **C-7 이식 규약 6종의 SSOT 는 클라우드전환-설계 §15 다** — 전부 실사고에서 나왔다(추측 항목 0). 특히 **§15-4: feature 당 최소 1회 실렌더 확인**. 이식 2건에서 사고가 2건 났고 **둘 다 정적 검사 전량 녹색**이었다(존재하지 않는 토큰이 회색으로 렌더 · 카드가 1글자 폭으로 붕괴). `--update-snapshots` 는 **깨진 결과를 정답으로 굳힌다** → 순서는 "이식 → 눈으로 확인 → 스냅샷 재생성"이고, **커버리지 0인 화면은 이식 _전에_ 스냅샷부터 만든다**.
...
- ⚠ **TS 7 · ESLint 10 은 생태계 차단이다**(2026-07-25 실측 · 우리 코드 문제 아님). TS 7.0.2 는 typecheck·build 를 통과하지만 `typescript-eslint` 전 버전이 TS7 을 지원하지 않고(peer `<6.1.0`) 실제로 하드 크래시한다 → **레이어 경계 린트(절대규칙 #2)가 죽는다.** ESLint 10 은 `eslint-plugin-jsx-a11y`(peer `^9`) 하나가 막는다. 둘 다 롤백했고 **다시 조사하지 말 것.** ⚠ 종전 이 자리가 _"재시도 조건은 SCA 원장에 적혀 있다"_ 라 가리켰는데 `audit-allowlist.json` 에 그 항목은 **없다**(2026-09-02 실측) — 정본은 **`.github/dependabot.yml` 의 `ignore` 블록 주석**이다(거기가 봇을 실제로 막는 자리이므로 근거도 거기 산다).
- ⭐ **단 「TS 가 5.9 에 묶였다」로 읽지 말 것 — TS 6 은 2026-09-02 에 취했다**(`O043`). 위 peer 범위가 `>=4.8.4 <6.1.0` 이라 **6 은 범위 안**이고 막히는 것은 7 뿐이다. web·server 를 **같은 커밋에서 함께** 올렸다 — `server/tsconfig.json` 의 `include` 가 web 소스 넷을 직접 물기 때문에 한쪽만 올리면 **같은 파일을 두 메이저가 컴파일한다**(봇은 트리마다 PR 을 내므로 그 조율은 사람이 한다).
